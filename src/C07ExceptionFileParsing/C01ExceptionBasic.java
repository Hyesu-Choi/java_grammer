...
//        예외처리 목적 : 1)사용자에게 적절한 에러 메시지 전달. 2)예외발생시 디버깅을 위한 로그 기록.
        ...
        try {  // 에러가 터질꺼같은 코드를 try구문에 넣어줌
            int head = Integer.parseInt(sc.nextLine());
            System.out.println("분모를 입력해주세요");
            int tail = Integer.parseInt(sc.nextLine());
            int result = head / tail;
            System.out.println("두 수의 나눈 값은 " + result);
//            catch를 통해 예상되는 예외클래스를 분기처리한다.
        } catch (ArithmeticException error) {
//            사용자에게 메시지 전달(가정. 지금은 그냥 출력으로..원레는 에러메세지로 전달해야함.)
            System.out.println("0으로 나누시면 안됩니다.");  // 사용자에서 에러 메세지 전달하는 용도.
//            시스템에 디버깅을 위한 로그 자세히 기록
            error.printStackTrace();  // 로그 남기는 용도
        } catch( NumberFormatException e) {
            System.out.println("문자를 입력하시면 안됩니다.");
            e.printStackTrace();
//            Exception은 모든 예외클래스의 조상클래스. 구체적인 예외 클래스가 catch부에 있다면 해당 부분으로 우선 분기처리 됨.
        } catch ( Exception e) {
