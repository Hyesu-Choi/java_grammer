...
//        스레드 생성
//        방법1. 스레드 클래스 상속방식
        Thread t1 = new MyThread();  // 스레드가 총 5개 만들어짐
        Thread t2 = new MyThread();
        Thread t3 = new MyThread();
        Thread t4 = new MyThread();
        Thread t5 = new MyThread();
//        각 스레드는 코드의 실행순서가 보장되지 않는 비동기적 실행(main에 할당횐 thread 포함)
        t1.start();
        t2.start();
        t3.start();
        t4.start();
...
//        구현객체의 run 메서드를 Thread객체가 가져다 사용
...
//        스레스 실행 1, 5, 4, 2, 3 뭐 약간 이런식으로 실행 순서가 보장 안됨.비동기식

//        멀티스레드 동시성 이슈 테스트
        for(int i=0; i<1000; i++) {
            Thread t1 = new Thread(() -> Library.borrow());
            t1.start();
//            join : 한 스레드에 작업이 모두 완료될떄까지, 다른 스레드를 생성하지 않는것. -> 사실상 단일스레드처럼 동작하므로, 성능저하 발생.
            t1.join();
        }
        Thread.sleep(30000);
        System.out.println(Library.getBookCount());
